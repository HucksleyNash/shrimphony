...
void main() {
  ...
  setUp(() async {
    ...
    client = JellyfinClient(JellyfinSession(
    ...
    server.listen((request) async {
      ...
      final items = query['ParentId'] == 'album'
          ? [
              {'Id': 'disc1track1', 'Name': 'Z first', 'Type': 'Audio',
                'ParentIndexNumber': 1, 'IndexNumber': 1},
              {'Id': 'disc1track2', 'Name': 'A second', 'Type': 'Audio',
                'ParentIndexNumber': 1, 'IndexNumber': 2},
              {'Id': 'disc2track1', 'Name': 'M third', 'Type': 'Audio',
                'ParentIndexNumber': 2, 'IndexNumber': 1},
            ]
          : query['IncludeItemTypes'] == 'Audio'
              ? List.generate(12, (i) => {
                  'Id': 'song-$i', 'Name': 'Song ${i.toString().padLeft(2, '0')}',
                  'Type': 'Audio',
                })
              : <Map<String, Object>>[];
  ...
  test('album playback preserves the server disc and track order', () async {
    ...
    expect(tracks.map((item) => item.id),
        ['disc1track1', 'disc1track2', 'disc2track1']);
  });

  test('artist detail collection actions receive every artist song', () async {
    final controller = AppController(JellyfinSessionStore())..client = client;
    try {
      ...
      final children = await controller.children(const JellyfinItem(
